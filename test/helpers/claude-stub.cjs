...
// (argv + relevant env + cwd) as a JSON line to $DOJO_STUB_LOG, prints canned
...
const log = process.env.DOJO_STUB_LOG;
if (log) {
  const rec = {
    argv,
    cwd: process.cwd(),
...
// can be exercised (comma-separated "name@marketplace" in DOJO_STUB_INSTALLED).
if (argv[0] === 'plugin' && argv[1] === 'list') {
  const seed = process.env.DOJO_STUB_INSTALLED || '';
  if (seed) process.stdout.write(seed.split(',').join('\n') + '\n');
}

process.exit(Number(process.env.DOJO_STUB_EXIT || 0));
