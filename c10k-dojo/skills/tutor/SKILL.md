---
name: tutor
description: Run the c10k-dojo Socratic tutoring loop for the learner's current step — frame the problem, quiz with AskUserQuestion, make the learner type the spine, review, benchmark, reflect. Use when the learner is working through the c10k-dojo web-server course or asks to start/continue a step.
---
...
3. **Design-quiz** — another AskUserQuestion: how should the fix be structured? (e.g. "which
   socket closes after fork?"). Again, distractors are the classic bugs. Steer to the right design.
...
7. **Reflect-quiz** — a final AskUserQuestion that cements the lesson (a comprehension check with a
   right answer, e.g. "it OOM'd at ~300 — why?"), then **point to the one next step and run
   `/c10k-dojo:next`.**
