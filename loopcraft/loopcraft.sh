#!/usr/bin/env bash
# Launch Claude Code for the loopcraft workshop, IN YOUR OWN PROJECT DIR:
#   - loads the loopcraft plugin (the tutor, hooks, commands),
...
# your workspace/ and the per-project .loopcraft/ state land there. The
# SessionStart hook auto-runs setup on first launch, then drops you into Step 1.
#
# Usage:
#   loopcraft.sh [project-dir] [claude args...]
#
#   loopcraft.sh                       # run the dojo in the CURRENT dir
#   loopcraft.sh /tmp/my-workshop      # create/enter that dir and run there
#   loopcraft.sh /tmp/ws --model llama3:8b   # ...with a local model
#   loopcraft.sh --model llama3:8b             # current dir + a model
#
# Tip: symlink this onto your PATH (e.g. ln -s "$PWD/loopcraft.sh" ~/bin/loopcraft)
# so you can run `loopcraft /tmp/ws` from anywhere.
set -euo pipefail
...
# be set HERE, before claude launches.
export CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false

echo "loopcraft → project: $(pwd)   plugin: $PLUGIN_DIR   (web disabled, prompt-suggestions off)" >&2
