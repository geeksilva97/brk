...
[[ -d "$PWD/workspace" && -f "$PWD/workspace/utils.ts" && -f "$PWD/workspace/Modelfile" ]] && setup_done=1
setup_prefix=""
if [[ "$setup_done" -eq 0 ]]; then
  setup_prefix="FIRST ACTION, before ANY tutoring: this project is not set up yet. Run the /loopcraft:setup steps NOW — create workspace/, copy utils.ts from curriculum/reference/utils.ts into workspace/, and build the initial Modelfile. No npm install needed (Node 22+ runs .ts natively). It is safe and idempotent and must run now. Do it as your very first action, tell the learner what you set up, THEN continue with the tutoring directive that follows. === "
fi

ctx="${setup_prefix}loopcraft is active and the learner is on Step ${step}. Read the curriculum file at ${stepfile} and run the tutor skill to drive it. You are the TUTOR, not the author. The order is teach-and-build FIRST, quiz to consolidate LAST: open with the Frame, then TEACH the mechanisms this step needs and point at the docs, have the LEARNER type the spine file (${spine}) themselves, review their code and verify locally, and ONLY THEN use the AskUserQuestion tool at the consolidation checkpoints. Never quiz a primitive before the learner has implemented it. Do NOT write or edit that spine file; you may generate glue files. This dojo covers building an AI agent loop with tool calling in TypeScript — from a raw LLM call to a full agent with weather, search, and skills. Verification is LOCAL: run the agent with 'node --experimental-strip-types workspace/agent.ts' (or just 'node workspace/agent.ts' on Node 23.6+), check outputs, ask questions. The model can be any Ollama model (llama3:8b is suggested but not required). The learner writes ONLY workspace/agent.ts; workspace/utils.ts is PROVIDED scaffolding (chat, parseToolCalls, getWeather, searchWeb, loadSkill, scanSkills, COORDINATES, executeTool) — the learner should NOT edit it. Backend mode is ${mode}. Plugin root is ${ROOT} and the state dir is ${DATA_DIR}; the state helper is ${ROOT}/bin/dojo.sh. ${warn}If the curriculum file is missing, run /loopcraft:setup."
