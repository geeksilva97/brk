...
case "$MODE" in
  ...
  bash)
    # Deny external HTTP(S) egress (allow localhost-style hosts only — that is where
    # the learner's own server runs and is tested with curl/nc).
    ...
    # Deny ad-hoc gem installs — the gem set is pinned (/demonkey:setup vendors it).
    if printf '%s' "$PAYLOAD" | grep -Eq '(gem[[:space:]]+install|bundle[[:space:]]+add)[[:space:]]'; then
      deny "Installing gems is off-limits — the dojo uses a pinned set (rack, protocol-http1, protocol-http). Build from stdlib + those. If you think you need another gem, that is usually a sign to reach for a Ruby primitive instead."
    fi
    # otherwise allow
    exit 0
    ;;
