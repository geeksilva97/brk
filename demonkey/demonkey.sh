#!/usr/bin/env bash
# Launch Claude Code for the demonkey workshop, IN YOUR OWN PROJECT DIR:
#   - loads the demonkey plugin (the tutor, hooks, commands),
...
# your workspace/ and the per-project .demonkey/ state land there. The
# SessionStart hook auto-runs setup on first launch, then drops you into Step 1.
#
# Usage:
#   demonkey.sh [project-dir] [claude args...]
#
#   demonkey.sh                       # run the dojo in the CURRENT dir
#   demonkey.sh /tmp/my-workshop      # create/enter that dir and run there
#   demonkey.sh /tmp/ws --model qwen2.5-coder:32b   # ...with a local model
#   demonkey.sh --model qwen2.5-coder:32b           # current dir + a model
#
# Tip: symlink this onto your PATH (e.g. ln -s "$PWD/demonkey.sh" ~/bin/demonkey)
# so you can run `demonkey /tmp/ws` from anywhere.
set -euo pipefail
...
echo "demonkey → project: $(pwd)   plugin: $PLUGIN_DIR   (web disabled, prompt-suggestions off)" >&2
