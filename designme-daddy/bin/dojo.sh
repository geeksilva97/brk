#!/usr/bin/env bash
# designme-daddy state helper. Reads/writes progress.json in the PROJECT's .designme-daddy/
...
#   dojo.sh kind  [step]        -> step kind (e.g. requirements|estimation|design)
...
#
# NOTE: this is a NO-CODE dojo (pure system-design conversation). Every step's
# "spine" column is "-" — there is no file the learner types. The spine machinery
# is retained from the dojo template (so the offline jail + state helper are
# identical), but the learner's "spine" here is their own DESIGN REASONING, scored
# verbally by the tutor — not a file on disk.
set -uo pipefail
