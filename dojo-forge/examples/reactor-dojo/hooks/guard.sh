...
case "$MODE" in
  ...
  bash)
    ...
    if [[ -n "$urls" ]]; then
      while IFS= read -r u; do
        ...
        case "$host" in
          ...
          *) deny "The dojo is offline. Outbound HTTP to ${host} is blocked. Use the mounted docs bundle (docs/INDEX.md); only localhost is reachable for testing your own code." ;;
        esac
      done <<< "$urls"
    fi
    # Deny ad-hoc dependency installs — the dependency set is pinned by /reactor-dojo:setup.
    if printf '%s' "$PAYLOAD" | grep -Eq '(gem[[:space:]]+install|bundle[[:space:]]+add|npm[[:space:]]+install|pip[[:space:]]+install|cargo[[:space:]]+add|go[[:space:]]+get)[[:space:]]'; then
      deny "Installing dependencies is off-limits — the dojo uses a pinned set provisioned by /reactor-dojo:setup. This dojo is stdlib-only — Ruby's Socket and IO.select are all you need. If you think you need another dependency, that is usually a sign to reach for a built-in primitive instead."
    fi
    # otherwise allow
    exit 0
    ;;
