#!/usr/bin/env bash
# loopcraft state helper. Reads/writes progress.json in the PROJECT's
# .loopcraft/ dir and the step table in curriculum/steps.tsv. No jq dependency
# — flat-JSON parsing.
#
# Usage:
...
#   dojo.sh kind  [step]        -> step kind (build|demo)
#   dojo.sh mode                -> backend mode
#   dojo.sh set-mode <mode>     -> set backend mode (local-jailed|anthropic-api)
