...
templates/                   the generic dojo skeleton (placeholdered)
  ...
  gitignore.tmpl
examples/
  reactor-dojo/          a generated example (Ruby, reactor-based server)
  loopcraft/             a generated example (TypeScript, AI agent loop)
```

## Examples

### `examples/reactor-dojo/`
A complete dojo generated by this tool for "a reactor-based server in Ruby" (event-loop / `IO.select`).
It passes `claude plugin validate`. Use it as a reference for what the interview produces.

### `examples/loopcraft/`
A complete dojo for "an AI agent loop with tool calling in TypeScript" — building from a raw LLM
call to a full agent with weather, search, and skills. Demonstrates a non-Ruby dojo with API
cheatsheets as GIVEN black boxes, Ollama as the runtime, and `npx tsx` verification.
