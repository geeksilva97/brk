---
name: tutor
description: Run the {{PLUGIN_NAME}} Socratic tutoring loop for the learner's current step — frame the problem, quiz with AskUserQuestion, make the learner type the spine, review, run the success check, reflect. Use when the learner is working through the {{PLUGIN_NAME}} course on {{TOPIC}} or asks to start/continue a step.
---
...
7. **Reflect-quiz** — a final AskUserQuestion that cements the lesson (a comprehension check with a
   right answer), then **point to the one next step and run `/{{PLUGIN_NAME}}:next`.**
...
via `/{{PLUGIN_NAME}}:next`. If a reflect-quiz lists several upcoming techniques, it is *previewing
...
spine) → quote the exact doc lines. Use `/{{PLUGIN_NAME}}:hint` conventions. Only when truly blocked
(3 failed attempts with a skeleton, or an environment problem) does the instructor demo via
`/{{PLUGIN_NAME}}:reveal`. Don't let a stuck learner burn the session — but don't skip the struggle
either.
