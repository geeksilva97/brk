...
case "$MODE" in
  ...
  bash)
    ...
    # Deny ad-hoc dependency installs — the dependency set is pinned by /{{PLUGIN_NAME}}:setup.
    if printf '%s' "$PAYLOAD" | grep -Eq '(gem[[:space:]]+install|bundle[[:space:]]+add|npm[[:space:]]+install|pip[[:space:]]+install|cargo[[:space:]]+add|go[[:space:]]+get)[[:space:]]'; then
      deny "Installing dependencies is off-limits — the dojo uses a pinned set provisioned by /{{PLUGIN_NAME}}:setup. {{DEPS_HINT}} If you think you need another dependency, that is usually a sign to reach for a built-in primitive instead."
    fi
    # otherwise allow
    exit 0
    ;;
