...
BIN_SRC="$REPO_DIR/bin/brk.js"
BIN_DIR="${BRK_BIN_DIR:-$HOME/.local/bin}"
LINK="$BIN_DIR/brk"
...
# (BRK_SKIP_PULL lets the test suite link without touching the repo's git state.)
if [ -d "$REPO_DIR/.git" ] && [ -z "${BRK_SKIP_PULL:-}" ]; then
