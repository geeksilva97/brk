...
A good answer covers: `+= 1` is read-modify-write — not atomic; threads interleave and lose updates
(a data race). The GVL doesn't make it atomic — it can switch threads *between* the read and the
write. Requests aren't being dropped; the requests run, the *updates* are lost.
...
A good answer covers: fibers — cheap, cooperative, one thread, no per-connection stack cost. More
threads won't help — we just hit that wall (stacks + contention). More processes are even heavier
than threads.
