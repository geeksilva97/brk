#!/usr/bin/env bash
# c10k-dojo state helper. Reads/writes progress.json in the PROJECT's .c10k-dojo/
...
#   dojo.sh kind  [step]        -> step kind (tcp|http|demo|bench)
...
write_progress() { # step completed spine tier mode
  printf '{ "step": %s, "completed": [%s], "spine_file": "%s", "tier": "%s", "mode": "%s" }\n' \
    "$1" "$2" "$3" "$4" "$5" > "$PROGRESS"
}
...
case "$cmd" in
  ...
  status) cat "$PROGRESS" ;;
  set-mode)
    write_progress "$(read_field step)" "$(read_completed)" "$(read_field spine_file)" "$(read_field tier)" "${1:-local-jailed}" ;;
  advance)
    ...
    write_progress "$next" "$comp" "$spine" "none" "$(read_field mode)"
