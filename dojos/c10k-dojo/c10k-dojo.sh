#!/usr/bin/env bash
# Launch Claude Code for the c10k-dojo workshop, IN YOUR OWN PROJECT DIR:
#   - loads the c10k-dojo plugin (the tutor, hooks, commands),
...
# your workspace/ and the per-project .c10k-dojo/ state land there. The
# SessionStart hook auto-runs setup on first launch, then drops you into Step 1.
#
# Usage:
#   c10k-dojo.sh [project-dir] [claude args...]
#
#   c10k-dojo.sh                       # run the dojo in the CURRENT dir
#   c10k-dojo.sh /tmp/my-workshop      # create/enter that dir and run there
#   c10k-dojo.sh /tmp/ws --model llama3:8b   # ...with a local model
#   c10k-dojo.sh --model llama3:8b           # current dir + a model
#
# Tip: symlink this onto your PATH (e.g. ln -s "$PWD/c10k-dojo.sh" ~/bin/c10k-dojo)
# so you can run `c10k-dojo /tmp/ws` from anywhere.
set -euo pipefail
...
echo "c10k-dojo → project: $(pwd)   plugin: $PLUGIN_DIR   (web disabled, prompt-suggestions off)" >&2
