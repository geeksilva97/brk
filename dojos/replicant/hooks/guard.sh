...
case "$MODE" in
  ...
  bash)
    # Deny external HTTP(S) egress (allow localhost-style hosts — the nodes talk over 127.0.0.1).
    ...
    if [[ -n "$urls" ]]; then
      while IFS= read -r u; do
        ...
        case "$host" in
          ...
          *) deny "The dojo is offline. Outbound HTTP to ${host} is blocked. Use the mounted docs bundle (docs/INDEX.md); only localhost is reachable for testing your own nodes." ;;
        esac
      done <<< "$urls"
    fi
    # Deny ad-hoc dependency installs — the dependency set is pinned by /replicant:setup.
    if printf '%s' "$PAYLOAD" | grep -Eq '(gem[[:space:]]+install|bundle[[:space:]]+add|npm[[:space:]]+install|pip[[:space:]]+install|cargo[[:space:]]+add|go[[:space:]]+get)[[:space:]]'; then
      deny "Installing dependencies is off-limits — the dojo uses a pinned set provisioned by /replicant:setup. The set is the sqlite3 gem (vendored) plus Ruby stdlib (socket, thread, json, digest, securerandom). If you think you need another dependency, that is usually a sign to reach for a built-in primitive instead."
    fi
    # otherwise allow
    exit 0
    ;;
  ...
  spine)
    ...
    case "$fp" in
      *"$spine")
        deny "${spine} is the learner's spine for this step — the lines that ARE the lesson. Do not write or edit it. Instead: explain the relevant docs, let the learner type it, then review by pointing at specific lines. (You may write the glue/scaffold files named in the step.)"
        ;;
    esac
    exit 0
    ;;
