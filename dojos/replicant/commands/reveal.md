...
1. Resolve the current step and its reference: look in `${CLAUDE_PLUGIN_ROOT}/curriculum/reference/`
   for the file named in the step's `reference:` pointer (if the step has one; many steps set it to
   `-` and this command should say there's no reference to show, prefer `/replicant:hint`).
...
Prefer `/replicant:hint` first. Only reveal after ~3 failed attempts with a skeleton, or for a
