#!/usr/bin/env bash
# Launch Claude Code for the replicant workshop, IN YOUR OWN PROJECT DIR:
#   - loads the replicant plugin (the tutor, hooks, commands),
...
# your workspace/ and the per-project .replicant/ state land there. The
# SessionStart hook auto-runs setup on first launch, then drops you into Step 1.
#
# Usage:
#   replicant.sh [project-dir] [claude args...]
#
#   replicant.sh                       # run the dojo in the CURRENT dir
#   replicant.sh /tmp/my-workshop      # create/enter that dir and run there
#   replicant.sh /tmp/ws --model qwen2.5-coder:32b   # ...with a local model
#   replicant.sh --model qwen2.5-coder:32b           # current dir + a model
#
# Tip: symlink this onto your PATH (e.g. ln -s "$PWD/replicant.sh" ~/bin/replicant)
# so you can run `replicant /tmp/ws` from anywhere.
set -euo pipefail
...
echo "replicant → project: $(pwd)   plugin: $PLUGIN_DIR   (web disabled, prompt-suggestions off)" >&2
