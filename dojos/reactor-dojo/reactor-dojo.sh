#!/usr/bin/env bash
# Launch Claude Code for the reactor-dojo workshop, IN YOUR OWN PROJECT DIR:
#   - loads the reactor-dojo plugin (the tutor, hooks, commands),
...
# your workspace/ and the per-project .reactor-dojo/ state land there. The
# SessionStart hook auto-runs setup on first launch, then drops you into Step 1.
#
# Usage:
#   reactor-dojo.sh [project-dir] [claude args...]
#
#   reactor-dojo.sh                       # run the dojo in the CURRENT dir
#   reactor-dojo.sh /tmp/my-workshop      # create/enter that dir and run there
#   reactor-dojo.sh /tmp/ws --model llama3:8b   # ...with a local model
#   reactor-dojo.sh --model llama3:8b           # current dir + a model
#
# Tip: symlink this onto your PATH (e.g. ln -s "$PWD/reactor-dojo.sh" ~/bin/reactor-dojo)
# so you can run `reactor-dojo /tmp/ws` from anywhere.
set -euo pipefail
...
echo "reactor-dojo → project: $(pwd)   plugin: $PLUGIN_DIR   (web disabled, prompt-suggestions off)" >&2
